...
export default function EditExaminerPage({
  ...
  useEffect(() => {
    if (!session) return;
    fetch(`${API_URL}/personnel/examiners/${params.id}`, {
      headers: { Authorization: `Bearer ${session.access_token}` },
    })
      .then((response) => (response.ok ? response.json() : null))
      .then((body: { data: Examiner } | null) => {
        if (!body) return;
        setExaminer(body.data);
        setFullName(body.data.full_name);
        setCpf(body.data.cpf);
        setAnacAccreditation(body.data.anac_accreditation ?? '');
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    const response = await fetch(`${API_URL}/personnel/examiners/${params.id}`, {
      method: 'PATCH',
      headers: {
    ...
    router.push(`/personnel/examiners/${params.id}`);
    router.refresh();
  }

  async function handleDelete(): Promise<void> {
    if (!confirm('Excluir este examinador? Essa ação não pode ser desfeita.')) return;
    setLoading(true);
    ...
    if (!response.ok) {
      setError('Não foi possível excluir o examinador.');
      return;
    }
  ...
  return (
    <main className="mx-auto max-w-lg p-8">
      ...
      <h1 className="mb-6 text-2xl font-semibold text-slate-900">Editar examinador</h1>
