...
export default function EditInstructorPage({
  ...
  useEffect(() => {
    if (!session) return;
    fetch(`${API_URL}/personnel/instructors/${params.id}`, {
      headers: { Authorization: `Bearer ${session.access_token}` },
    })
      .then((response) => (response.ok ? response.json() : null))
      .then((body: { data: Instructor } | null) => {
        if (!body) return;
        setInstructor(body.data);
        setFullName(body.data.full_name);
        setCpf(body.data.cpf);
        setAnacRegistration(body.data.anac_registration ?? '');
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    const response = await fetch(`${API_URL}/personnel/instructors/${params.id}`, {
      method: 'PATCH',
      headers: {
    ...
    router.push(`/personnel/instructors/${params.id}`);
    router.refresh();
  }

  async function handleDelete(): Promise<void> {
    if (!confirm('Excluir este instrutor? Essa ação não pode ser desfeita.')) return;
    setLoading(true);
