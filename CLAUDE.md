...
- ✅ **Qualificação automática (RN-05) + entrada manual** — `Course.modality` (`TEORICO`/`PRATICO`/`MISTO`, migração real aplicada no Supabase) e `Qualification.certificate_id` adicionados ao schema. Validado ponta a ponta contra a infra real: curso `TEORICO` completo (matrícula → frequência → exame teórico aprovado) → `POST /certificates` emite o certificado **e cria a `Qualification` automaticamente** (`course_id`/`certificate_id`/`qualification_code` corretos, confirmado via `execute_sql`); curso `PRATICO` completo (mesma sequência, exame prático) → certificado emitido **sem** criar qualificação; `POST /qualifications` (entrada manual) criada com sucesso pra esse mesmo curso prático; duas tentativas de IDOR (`student_id`/`course_id` inexistentes) rejeitadas com 404. Toda a árvore de fixtures (cursos, matrículas, exames, frequência via `Segment`→`Module`→`Unit`→`SubUnit`→`Lesson` inserida por `execute_sql`, já que não há endpoint pra essa hierarquia) removida ao final.
- ✅ **Documents CRUD + filtro de Pessoal + toggle ativo no cadastro de aluno + botões "voltar"** — `GET :id`/`PATCH`/`DELETE` adicionados a `Document` (só tinha create/list/versions), com telas `new`/`[id]/edit`; filtro Ativo/Inativo/Todos adicionado a `/personnel` (mesmo padrão de `/students`, aplicado a instrutores e examinadores simultaneamente via um `searchParams.status` só); campo `active` (opcional, default `true` do Prisma) adicionado a `CreateStudentDto`/`createStudent` e exposto no form de criação de aluno (antes só existia no de edição); link "← Voltar para X" adicionado nas telas `new`/`[id]/edit` de Students, Personnel (instructors/examiners), Courses, Clients, Contracts e Documents. Validado ponta a ponta: Documents via HTTP real (create → get → patch título/categoria/status → delete → get pós-delete 404); Student `active` via HTTP real (`active: false` explícito, sem o campo → confirma default `true` do Prisma); filtro de Personnel e todos os 13 botões "voltar" (7 telas `new` + 6 telas `edit`) via Playwright real contra o Supabase (login real do usuário demo, Chromium headless, zero erros de console) — Playwright instalado com `--no-save` só para a validação e removido depois, não é dependência do projeto.
- ✅ **Emissão automática de certificado (RN-05) + download/upload de Documentos + ajustes finos de UI** — `Course.min_passing_score` adicionado ao schema (migração real aplicada); `registerExam`/`registerAttendance` agora tentam emitir o certificado automaticamente após cada gravação (`AcademicService.tryAutoIssueCertificate`), com guarda contra emissão duplicada tanto no caminho automático quanto no manual (`POST /certificates` agora rejeita com 400 se o enrollment já tem certificado). `StorageService.createSignedUrl` + `GET /documents/:id/download` adicionados para permitir baixar o arquivo de um bucket privado; tela de criação de Documento agora redireciona para a edição, onde há upload de nova versão (multipart, reutilizando `POST /documents/:id/versions`) e um botão "Baixar documento". Checkbox ativo/inativo adicionado às telas de criação de Instructor, Examiner e Client (antes só existiam na edição); link "← Voltar para Qualificações" adicionado em `/qualifications/new` (fila que ficou de fora do lote anterior). Validado ponta a ponta: curso `TEORICO` com `min_passing_score=70` → matrícula → frequência → exame com `score=85` e **sem `result` explícito** → certificado emitido sozinho, com `Qualification` vinculada, confirmado via `execute_sql`; segunda tentativa manual de `POST /certificates` para o mesmo enrollment rejeitada com 400; criação de Documento → upload real de arquivo → download retorna uma signed URL cujo conteúdo bate exatamente com o que foi enviado (via HTTP direto e também via Playwright real, clicando nos botões de upload/download no navegador); checkboxes ativo/inativo e o link de Qualificações confirmados via Playwright. Dados de teste (curso, matrícula, hierarquia `Segment`→`Lesson`, documentos, certificados, qualificação) removidos ao final — restam apenas dois arquivos de teste órfãos no bucket privado `regulatory-docs` (poucos bytes de texto, sem linha de `Document`/`DocumentVersion` apontando pra eles), já que o Supabase bloqueia `DELETE` direto em `storage.objects` via SQL e não há uma Storage API MCP disponível nesta sessão para removê-los.
- ✅ **Fix: flicker no `next dev` + download de Documentos travado** — `reactStrictMode: false` em `apps/web/next.config.js` (StrictMode remontava cada tela em dev, resetando o estado e disparando cada fetch duas vezes — confirmado via rede, `GET /students/:id` chegava 2x ao abrir a edição, e some depois do fix); `handleDownload` em `documents/[id]/edit` corrigido pra abrir a aba **antes** do `await fetch`, não depois (popup blocker barrava silenciosamente quando `window.open` vinha depois de um `await`). Validado via Playwright real: mesma tela de edição de aluno passou de 2 requisições pra 1; clique em "Baixar documento" agora abre a aba de forma confiável (confirmado abrindo uma segunda página no browser).
- ✅ **Formulário único de criação de Documento** — `/documents/new` ganhou os campos `status` e `arquivo` (antes só tinha título/categoria; status era fixo em `EM_ELABORACAO` e o upload só existia na edição). `CreateDocumentDto`/`createDocument` aceitam `status` opcional agora. Ao enviar, o form cria o `Document` e, se um arquivo foi escolhido, sobe a v1 na sequência (mesmo endpoint multipart da edição) antes de ir pra listagem. Nota de produto encontrada nesse teste: subir uma versão sempre marca o documento como `EM_REVISAO` (comportamento de `addVersion`, não é bug novo) — por isso o form mostra um aviso quando um arquivo é escolhido, explicando que o status final vai ser "Em revisão" independente do que for selecionado no campo `status`. Validado ponta a ponta via Playwright real: os 4 campos presentes, submissão única cria o documento com título/categoria corretos e a versão 1 anexada, aparece na listagem.
...
- **"Cannot find the middleware module"** — sintoma de cache de build (`apps/web/.next`) corrompido/desatualizado, tipicamente depois de reiniciar o `next dev` várias vezes numa sessão longa (`middleware-manifest.json` fica com `"middleware": {}` vazio mesmo com `src/middleware.ts` existindo). Fix: parar o processo, apagar `apps/web/.next`, subir `dev:web` de novo.
