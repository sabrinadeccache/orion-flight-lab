...
1. ✅ **CRUD de frontend** (ver §18 para o mapa completo) — item concluído nesta rodada: Certificates ganhou **Criar + Detalhe + Download** (sem editar/excluir, mesmo modelo de trilha de auditoria de SGQ/SGSO — decisão explícita do usuário, ver §12). Isso fecha o último gap de CRUD de frontend listado no §18: Clients/Contracts/Documents já tinham CRUD completo, Qualifications já tinha entrada manual (`/qualifications/new`), SGQ/SGSO já tinham o modelo Listar + Criar + ações de estado (com edição em AuditProgram/Audit/Hazard/Risk), e CRM/Financial já tinham Listar + Criar + GET :id + edição.
