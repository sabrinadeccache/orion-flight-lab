...
- **"Cannot find the middleware module"** — sintoma de cache de build (`apps/web/.next`) corrompido/desatualizado, tipicamente depois de reiniciar o `next dev` várias vezes numa sessão longa (`middleware-manifest.json` fica com `"middleware": {}` vazio mesmo com `src/middleware.ts` existindo). **Mitigado automaticamente desde esta sessão**: `apps/web/scripts/check-next-cache.js`, registrado como script `predev` em `apps/web/package.json`, roda antes de todo `next dev` (inclusive via `npm run dev:web` da raiz), detecta esse exato mismatch e apaga `apps/web/.next` sozinho só quando necessário (cache saudável não é tocado, então o rebuild incremental do dia a dia continua rápido). Se o erro voltar a aparecer mesmo assim, o fix manual continua válido: parar o processo, apagar `apps/web/.next`, subir `dev:web` de novo — e vale investigar por que o guard não pegou o caso (ver o script).
